...
<strong class="active-color">New Shoes Updated</strong>
</div>
</div>
...
<?php foreach ($products_shoes as $key => $shoes): ?>
<div class="grid-100">
...
<a class="product-img" href="/product/detail/{!! $shoes['id'] !!}">
<img src="{{asset('/').'/'.$shoes['images']}}" alt="Pablo Coelho jacket"/>
</a>
</div>
<div class="grid-50 tablet-grid-45 mobile-grid-65 product-description">
<h3 class="product-title subheader-font">
<a href="/product/detail/{!! $shoes['id'] !!}" class="dark-color active-hover">
<strong>{!! $shoes['name'] !!}</strong>
</a>
</h3>
<a href="/product/detail/{!! $shoes['id'] !!}" class="product-category middle-color dark-hover">{!! $shoes->name !!}’s Suit Jacket</a>
<p class="dark-color hide-on-mobile">{!! $shoes['intro_short'] !!}</p>
</div>
...
<a href="cart.html" class=" addcart button-dual-left middle-gradient dark-gradient-hover" data-pid="{!! $shoes['id']!!}" data-uid="Session::get('id')" data-price="{!! $shoes['sale'] !!}">
